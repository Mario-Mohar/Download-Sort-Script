...
def cleanup_old_backups(desktop_path, keep_count=5):
    """Löscht alte Backups und behält nur die neuesten"""
    backup_pattern = desktop_path / "Downloads_Backup_*"
    backup_folders = sorted(Path(desktop_path).glob("Downloads_Backup_*"), key=os.path.getctime, reverse=True)
    ...
    if len(backup_folders) > keep_count:
        for old_backup in backup_folders[keep_count:]:
            try:
                ...
                logging.warning(f"Konnte altes Backup nicht löschen {old_backup}: {e}")
...
def create_backup():
    ...
    try:
        ...
        # Alte Backups aufräumen (behalte nur die 5 neuesten)
        cleanup_old_backups(desktop_path, 5)
...
def sort_downloads():
    ...
    if backup_path:
        ...
        logging.info("Sie können das Backup auf dem Desktop finden, falls Sie Dateien wiederherstellen müssen.")
    else:
    ...
    if backup_path:
        logging.info(f"Backup verfügbar unter: {backup_path}")

if __name__ == "__main__":
    setup_logging()
    logging.info("Download-Sortierung mit Backup gestartet")
    sort_downloads()
